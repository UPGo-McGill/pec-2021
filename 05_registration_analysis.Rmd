...
Since the start of the registration, a total of `r n_reg_property` listings were found on STR platforms. Consdering the `r total_licenses_all` license applications, the compliance rate of STRs stands at `r p_registration_compliance`, indicating that about half of STRs in the County have either complied or are in the process of complying with the new registration system. 

Out of the `r total_licenses_all` total applications received, `r valid_STA` were approved, with `r n_valid_STA_bc` of them occurring prior to the moratorium on STA licenses. The ward that received the most applications was Picton with `r picton_n_license`, followed by Hallowell with `r hallo_n_license`, and Wellington with `r well_n_license` (Table \@ref(tab:tab-5-1)). `r ids_sta_pid` of the `r valid_STA_bc` short-term accommodations with valid licenses had one or more STR property within a 200-meter buffer of their address (Airbnb and VRBO obfuscate listing locations by shifting them randomly up to 200 m). 
...
proportion_home_sales_STA %>% 
  ggplot()+
  geom_col(aes(x=year, y=perc_adjusted), fill=col_palette[1], alpha=0.3)+
  geom_col(aes(x=year, y=perc), fill=col_palette[6], alpha=0.3)+
  geom_line(aes(x=year, y=perc_adjusted), color=col_palette[1])+
  geom_line(aes(x=year, y=perc), color=col_palette[6])+
...
(ref:fig-5-2) _Percentage of home sales for short-term rentals per year, real numbers (orange) and adjusted with compliance rate (yellow)_

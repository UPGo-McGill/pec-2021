...
```{r redistribute_town_townships, include = F}
pool_tt <- 
daily %>% 
...
waffle_df <- 
tibble(graph = "first", value = full_time_tt, group = "Removed FT properties") %>% 
  add_row(graph = "second", value = home_sharing_tt, group = "Existing home sharing") %>% 
  add_row(graph = "second", value = new_home_sharers_tt, group = "New home sharing") %>%
  add_row(graph = "third", value = full_time_tt-grandfathered_tt, group = "Removed FT properties") %>% 
  ...
  add_row(graph = "fourth", value = new_home_sharers_after_gf_tt, group = "New home sharing") %>% 
  ...
  mutate(group = factor(group, levels = c("Remaining FT properties", "New home sharing",
                           "Removed FT properties", "Existing home sharing"))) %>%
  arrange(group)

cols <- c("Remaining FT properties" = col_palette[1],
          "Removed FT properties" = col_palette[5], 
...
waffle_df <- 
tibble(graph = "first", value = full_time_t, group = "Removed FT properties") %>% 
  add_row(graph = "second", value = home_sharing_t, group = "Existing home sharing") %>% 
  add_row(graph = "second", value = new_home_sharers_t, group = "New home sharing") %>%
  add_row(graph = "third", value = full_time_t-grandfathered_t, group = "Removed FT properties") %>% 
  add_row(graph = "third", value = grandfathered_t, group = "Remaining FT properties") %>% 
  add_row(graph = "fourth", value = home_sharing_t, group = "Existing home sharing") %>% 
  add_row(graph = "fourth", value = new_home_sharers_after_gf_t, group = "New home sharing") %>% 
  add_row(graph = "antes_first", value = full_time_t, group = "Remaining FT properties") %>% 
  add_row(graph = "antes_second", value = home_sharing_t, group = "Existing home sharing") %>% 
  mutate(value = value/10) %>% 
  mutate(group = factor(group, levels = c("Remaining FT properties", "New home sharing",
                           "Removed FT properties", "Existing home sharing"))) %>%
  arrange(group)

...
```{r owner_afford, cache = TRUE, cache.lazy = FALSE, cache.extra = mtime(here("output", "str_processed.qsm"))}
...
figure_2_5_fun <- function(regular = "", condensed = "") {
  ...
  left_map <-
    ...
    guides(fill = guide_colourbar(title = "Homeowner\nhousing stress",
                                  title.vjust = 1)) +
    gg_bbox(DA_housing) +
    theme_void() +
    theme(text = element_text(family = regular, face = "plain"),
...
The housing sales trends described above tell only half the story of homeownership affordability in Prince Edward County. The other half is the ability of County residents to afford elevated housing prices. The most recent reliable data on Canadian incomes comes from the 2016 Census, which means that it will not have captured changes in income in the last five years. With that caveat, it remains the case that homeownership affordability has deteriorated substantially in the County. Based on the standard affordability rule that the shelter cost-income ratio should not exceed 30%, 17.3% of County homeowners are considered housing stressed. This percentage varies between ward, with Bloomfield exhibiting the most homeowner stress, with  `r p_thirty_owner_bloomfield` of owner-occupied households spending more than 30% of their incomes on housing, and Athol exhibiting the least housing-stressed households, with `r p_thirty_owner_athol`. Figure \@ref(fig:fig-2-5) shows the geographic distribution of owner households experiencing housing stress by dissemination area (left) and by ward (right).
...
```
(ref:fig-2-5) _Homeowner housing stress by dissemination area (L) and ward (R)_

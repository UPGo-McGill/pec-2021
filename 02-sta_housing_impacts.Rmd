...
(ref:fig-2-2) _Percentage of homeowners' housing stress by dissemination area (R) and ward (L)_
...
home_sales %>%
  ...
  ggplot() +
  geom_col(aes(year_month, n), fill = col_palette[5], lwd=0)+
...
(ref:fig-2-3) _Monthly number of property sales in Prince Edward County_
...
(ref:fig-2-4) _Year-over-year changes in monthly real estate sales and average sale prices_
...
(ref:fig-2-5) _Monthly averages of days on the market_
...
(ref:fig-2-6) _Monthly difference between asking and sale prices_
...
north_avg_price <- 
  sale_price_by_ward %>% 
  st_drop_geometry() %>%
  arrange(desc(avg_sale_price_2021)) %>% 
  slice(1) %>% 
  pull(avg_sale_price_2021) %>% 
  scales::dollar(10)

athol_avg_price <- 
  sale_price_by_ward %>% 
  st_drop_geometry() %>%
  arrange(avg_sale_price_2021) %>% 
  slice(1) %>% 
  pull(avg_sale_price_2021) %>% 
  scales::dollar(10)
...
figure_2_9_fun <- function(regular = "", condensed = "") {
...
if (build_figures) {
  ggsave(here("output", "figures", "figure_2_9.pdf"), 
         plot = figure_2_9_fun("Futura", "Futura Condensed"), 
         width = 8, height = 5, units = "in", useDingbats = FALSE)
  
  extrafont::embed_fonts(here("output", "figures", "figure_2_9.pdf"))
}

```

On average in 2021, there were `r FREH_2021` FREH listings in PEC, and `r GH_2021` more housing units which were operating as ghost hostels. In total, therefore, short-term rentals are taking `r housing_loss_2021` housing units off of PEC’s long-term market (Figure \@ref(fig:fig-2-9)). Notably, while the number of active daily listings declined by `r active_yoy_listings` between 2019 and 2021, the number of housing units which STRs took off of PEC’s housing market decreased at a much sharper rate. There were `r housing_loss_change` fewer dedicated STRs in 2021 compared to 2019, from `r housing_loss_2019` to `r housing_loss_2021`. This high decline in commercially-operated STRs reflects that the pandemic has disproportionately affected commercial operators in the County.

``` {r fig-2-9, include = TRUE, fig.cap = '(ref:fig-2-9)', fig.align = "center"}

figure_2_9_fun()

```
(ref:fig-2-9) _Housing units converted to dedicated STRs in the Prince Edward County_
...
figure_2_11_fun <- function(regular = "", condensed = "") {
...
if (build_figures) {
  ggsave(here("output", "figures", "figure_2_11.pdf"), 
         plot = figure_2_11_fun("Futura", "Futura Condensed"), 
         width = 8, height = 5, units = "in", useDingbats = FALSE)
  
  extrafont::embed_fonts(here("output", "figures", "figure_2_11.pdf"))
}

```

On average, for the pre-Covid years, half of entire-home listings (`r eh_housing_loss_pre_covid`) and of the private-room listings (`r pr_housing_loss_pre_covid`) were taking housing off the long-term market. On average, in 2021, only a fourth of entire-home listings (`r eh_housing_loss`) but back to haf of private-room listings (`r pr_housing_loss`) were being operated in a full-time or close-to-full-time fashion (Figure \@ref(fig:fig-4-3)). The yearly up-and-down pattern on Figure \@ref(fig:fig-2-11) is that, in winter, a lower percentage of FREH listings are removed from the platform than for less commercial listings. In addition, there are more active listings in the summer, shrinking the percentage of entire home/apt STR that contributes to long-term housing-loss even if their number in absolute was stable before the pandemic.
...
``` {r fig-2-11, include = TRUE, fig.cap = '(ref:fig-2-11)', fig.align = "center"}

figure_2_11_fun()

```

(ref:fig-2-11) _The percentage of active STR listings contributing to long-term housing loss each day in PEC (14-day average)_

...
sale_price <- 
  home_sales %>% 
  st_drop_geometry() %>% 
  mutate(yearmon = tsibble::yearmonth(sale_date)) %>% 
  group_by(yearmon) %>% 
  summarize(avg_price = mean(sale_price, na.rm=T)) %>% 
  mutate(yearmon = as.Date(yearmon)) %>% 
  ...
  select(yearmon, monthly_payments)
...
nights_booked_needed %>% 
  filter(yearmon >= "2018-01-01", yearmon < "2019-01-01") %>%
  summarize(mean(nights_needed)) %>% 
  pull() %>% 
  round(., digits = 2)

needed_to_afford_2021 <- 
nights_booked_needed %>% 
  filter(yearmon >= "2021-01-01") %>%
  summarize(mean(nights_needed, na.rm =T)) %>% 
  pull() %>% 
  round(., digits = 2)
...
nights_booked_needed %>%
  filter(yearmon >= "2018-01-01") %>% 
  ggplot()+ 
  geom_line(aes(yearmon, y = nights_needed), size = 1, color = col_palette[1])+ 
  geom_smooth(aes(yearmon, y = nights_needed), se = F, color = col_palette[2])+
...
```
(ref:fig-3-3) _Number of nights needed to afford monthly mortgage payment_

...
daily_variation <- 
  daily %>% 
  filter(housing, status != "B", date >= "2015-12-16", date != "2019-02-29") %>% 
  group_by(date) %>% 
...
EW_breakdown <- 
  ...
  summarize(active_listings = mean(n[date >= start_2019]),
            active_2019 = mean(n[date < start_2019]),
            active_growth = (active_listings - active_2019) / active_2019,
            annual_rev = sum(revenue[date >= start_2019]),
            rev_2019 = sum(revenue[date < start_2019]),
...
Active daily listings are listings which were displayed on Airbnb or Vrbo on a given day, and were either reserved or available for a reservation. It is the most reliable means of determining the overall size of the STR market in a location, particularly with respect to change over time. In 2020, there was an average of `r avg_listings_active` active daily listings (Figure \@ref(fig:fig-1-1)) operated by an average of `r avg_hosts` hosts. These hosts collectively earned `r annual_rev`—an average of `r avg_rev_per_listing` per daily active listing or `r avg_rev_per_host` per active host. These numbers are only a portion of Prince Edward County's STR market from before the pandemic. In 2019, the total STR market revenue amounted to `r annual_rev19`, with an average of `r avg_listings_active19` active listings on a given day operated by `r avg_hosts20` hosts. In 2019, the average listing earned `r avg_rev_per_listing19` and the average host, `r avg_rev_per_host19`. 
...
There was also a daily average of `r avg_listings_blocked` listings which were visible on the Airbnb and Vrbo websites but were blocked by the host from receiving reservations (which is more than the previous year, which had on average `r avg_listings_blocked19` blocked listings on a given). The presence of these listings can erroneously suggest that a city’s STR market is larger than it is. When these blocked but inactive listings are included, the average listing earned `r avg_revenue_all_listings` in 2020, and the average host earned `r avg_revenue_all_hosts`. Finally, there was a daily average of `r non_housing_listings` listings that were not located in private housing units (B&Bs, hotels, etc.), which have been excluded from the analysis in this report. All the material which follows pertain to short-term rentals located in Prince Edward County’s residential housing stock.
...
Active daily listings peaked in `r highest_activity_date` at `r highest_activity_amount`, and have since declined. Due to the Covid-19 pandemic, there were `r active_yoy_listings` fewer active listings on average in 2020 than in 2019, and `r active_yoy_revenue` less revenue. Prior to 2020, Prince Edward County's STR market was growing rapidly—active listings and revenue respectively increased by `r active_yoy_listings19` and `r active_yoy_revenue19` from 2018 to 2019. It is likely that this trend would have continued in 2020 in the absence of the pandemic. In general, the number of STR listings available in Prince Edward County follows a strongly seasonal pattern, with the bulk of activity happening in the summer (June-July-August) months (Figure \@ref(fig:fig-1-1)). Detailed information about seasonality in Prince Edward County will be presented in the next chapter.
...
EW_breakdown %>% 
  ...
  kbl(caption = "Wards with at least 20 daily active listings in 2020",
               align = "lrrrrr") %>% 
...
In 2020 entire-home listings accounted for `r eh_listings` of all daily active listings, and `r eh_revenue` of total host revenue. (Private rooms accounted for nearly all of the remainder.) Moreover, the dominance of entire-home listings in Prince Edward County’s STR market is increasing over time.
...
Since 2018, the number of active STRs listings has been steadily decreasing in Prince Edward County. Figure \@ref(fig:fig-1-3) shows the change in active listings and revenue relative to one year earlier, which is a convenient way to remove seasonal variation to identify underlying growth trends. The figure indicates that, since 2018, the number of daily active listings has been declining. Revenues, on the other hand, have experienced growth since the beginning of 2019. With the exception of the winter 2020 months, Prince Edward County's STR revenues have been higher than the year before. The pandemic had a clear negative effect on the number of active daily listings being displayed as reserved or available on STR platforms as well as revenues. We observe higher revenues for the summer of 2021 than we did in 2020, indicating an increase in tourist activity starting 2021.
...
Overall, the year-over-year change in average active listings from 2017 to 2018  was `r listing_growth_2018` and the year-over-year change from 2018 to 2019 was `r listing_growth_2019`. In 2020, because of the Covid-19 pandemic, the year-over-year change in active daily listings is `r listing_growth_2020`.
...
reservation_change_2020 <- 
  ...
  filter(housing, status == "R", date >= start_2021 - years(1),
         date <= end_2021) %>%
  group_by(year_2020 = date >= start_2021) %>%
  summarize(n = n()) %>%
  summarize(change = (n[2] - n[1]) / n[1]) %>% 
  pull(change) %>% 
  scales::percent(0.1)
...
reservation_change_2019 <- 
  ...
  filter(housing, status == "R", date >= start_2020 - years(1),
         date <= end_2020) %>%
  group_by(year_2019 = date >= start_2020) %>%
  summarize(n = n()) %>%
  summarize(change = (n[2] - n[1]) / n[1]) %>% 
  pull(change) %>% 
  scales::percent(0.1)
...
active_yoy_revenue_2020 <- 
  ...
  filter(housing, status == "R", date >= start_2020,
         ...
         (date <= "2019-07-31" | date >= start_2020 + years(1)),
         date != "2020-02-29") %>%
  group_by(year_2020 = date >= start_2020 + years(1)) %>%
...
reserved_nights_decrease_2020 <- 
  ...
  filter(housing, status == "R", date >= start_2021 - years(1), 
         date <= end_2021) %>% 
  count(date_2019 = date >= start_2021) %>% 
  summarize(growth = (n[2] - n[1]) / n[1]) %>% 
  pull(growth) %>% 
  scales::percent(0.1)
...
In 2020, `r ml_listings` of active listings in Prince Edward County were multilistings, earning `r ml_revenue` of total host revenue. Multilistings had been growing steadily since 2018, both in terms of listings and revenue percentage, until the Covid-19 pandemic, when their proportion dropped somewhat (Figure \@ref(fig:fig-1-7)). Still, through the fall of 2020 multilistings were earning close to 5 out of every 10 dollars on STR platforms in Prince Edward County. 

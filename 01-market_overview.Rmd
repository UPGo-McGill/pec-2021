...
active_2021 <- 
  daily %>%
  filter(housing, status %in% c("R", "A"), date <= end_2021, 
         date >= start_2021) %>%
  pull(property_ID) %>% 
  unique()
...
revenue_2021 <-
  daily %>%
  filter(housing, status == "R", date <= end_2021, 
         date >= start_2021) %>%
  group_by(property_ID) %>%
...
EW_breakdown <- 
  daily %>% 
  filter(housing, status != "B", date >= start_2021 - years(1), 
         date <= end_2021) %>% 
  group_by(date, ward) %>% 
  summarize(n = n(),
  ...
  summarize(active_listings = mean(n[date >= start_2021]),
            active_2020 = mean(n[date < start_2021]),
            active_growth = (active_listings - active_2020) / active_2020,
            annual_rev = sum(revenue[date >= start_2021]),
            rev_2020 = sum(revenue[date < start_2021]),
            rev_growth = (annual_rev - rev_2020) / rev_2020,
            .groups = "drop") %>% 
...
listing_type_breakdown <- 
  daily %>% 
  filter(housing, status != "B", date >= start_2021 - years(1), 
         date <= end_2021 - years(1)) %>% 
  group_by(listing_type) %>% 
  summarize(active_2020 = n() / 365) %>% 
  left_join(listing_type_breakdown, .) %>% 
  mutate(pct_listing_growth = (active_listings - active_2020) / active_2020) %>% 
  select(-active_2020)

# Host revenue table for calculations and table
host_rev <-
  daily %>%
  filter(housing, date >= start_2021, date <= end_2021, 
...
**There were `r avg_listings_active` active STR listings in Prince Edward County housing units in the 2020-2021 season, which collectively earned `r annual_rev`. This is a dramatic decline in comparison with the 2019-2020; the COVID-19 pandemic reduced reservations by XX.X%. One fifth of all listings are located in Picton. The quasi totality (`r eh_listings`) of Prince Edward County’s STR listings are entire homes, and entire homes are responsible for `r eh_revenue` of all revenue. Revenue is concentrated unevenly among hosts, with the top 10% earning almost half (`r host_rev_top_10`) of revenue, and the top 1% earning `r host_rev_top_1`. Commercial operators who control multiple STR listings account for `r ml_listings` of listings and their revenue amount to `r ml_revenue` of total revenue.**
...
avg_rev_per_listing <- 
  (sum(revenue_2021$revenue_2021) /
    daily %>% 
    filter(housing, status != "B", date >= start_2021, date <= end_2021) %>% 
    count(date) %>% 
...
avg_rev_per_listing20 <- 
  (sum(revenue_2020$revenue_2020) /
    daily %>% 
    filter(housing, status != "B", date >= start_2020, date <= end_2020) %>% 
    count(date) %>% 
...
Active daily listings are listings which were displayed on Airbnb or Vrbo on a given day, and were either reserved or available for a reservation. It is the most reliable means of determining the overall size of the STR market in a location, particularly with respect to change over time. In the 2020-2021 season there was an average of `r avg_listings_active` active daily listings (Figure \@ref(fig:fig-1-1)) operated by an average of `r avg_hosts` hosts. These hosts collectively earned `r annual_rev`—an average of `r avg_rev_per_listing` per daily active listing or `r avg_rev_per_host` per active host. The STR market in Prince Edward County had already started to bounce back from the Covid-related decline in revenue in 2019-2020 (in 2019-2020, the total STR market revenue amounted to `r annual_rev20`), albeit the number of listings remain lower. For comparison, the 2019-2020 period had on average `r avg_listings_active20` active listings on a given day operated by `r avg_hosts20` hosts. The average listing earned `r avg_rev_per_listing20` and the average host `r avg_rev_per_host20`. 
...
There was also a daily average of `r avg_listings_blocked` listings which were visible on the Airbnb and Vrbo websites but were blocked by the host from receiving reservations (which is more than the previous year, which had on average `r avg_listings_blocked20` blocked listings on a given). The presence of these listings can erroneously suggest that a city’s STR market is larger than it is. When these blocked but inactive listings are included, the average listing earned `r avg_revenue_all_listings` in 2021, and the average host earned `r avg_revenue_all_hosts`. Finally, there was a daily average of `r non_housing_listings` listings that were not located in private housing units (B&Bs, hotels, etc.), which have been excluded from the analysis in this report. All the material which follows pertain to short-term rentals located in Prince Edward County’s residential housing stock.
...
# Active listing YOY change in 2021
active_yoy_change <- 
  daily %>% 
  filter(housing, status != "B", date >= start_2021 - years(1), 
         date <= end_2021) %>% 
...
Active daily listings peaked in `r highest_activity_date` at `r highest_activity_amount`, and have since declined. The number of active listings have decreases by `r active_yoy_listings` on average for the 2020-2021 period compared to the 2019-2020 period, which had already declined by  `r active_yoy_listings20` compared to 2018-2019. However, despite the disruption caused by the Covid-19 pandemic on tourism, Prince Edward County's STR revenues have increased for both time periods affected by the pandemic. For the 2020-2021 period, revenues increased by `r active_yoy_revenue` compared to the year before, despite the declining number of active listings. For 2019-2020, revenues had increased by `r active_yoy_revenue20`. In general, the number of STR listings available in Prince Edward County follows a strongly seasonal pattern, with the bulk of activity happening in the summer (June-July-August) months (Figure \@ref(fig:fig-1-1)). Detailed information about seasonality in Prince Edward County will be shared in the next chapter.
...
max_per_cap <- 
  EW_breakdown %>% 
  filter(active_listings > 100, 
         !active_listings %in% c("Bloomfield", "Wellington")) %>% 
  pull(listings_pct_dwellings) %>% 
  max() %>% 
  scales::percent(0.1)
...
STR activity in Prince Edward County is highly concentrated in the towns, as opposed to the townships (Figure \@ref(fig:fig-1-2)). In particular, Picton accounts for `r picton_listing_pct` of all listings, and an even higher share of host revenue (`r picton_rev_pct`). Picton is followed by Hallowell (`r hallowell_listing_pct` of active listings and `r hallowell_rev_pct` of revenue) and Wellington (`r wellington_listing_pct` of active listings and `r wellington_rev_pct` of revenue). When measured in per-capita terms, Bloomfield has by far the highest concentrations of STRs, at `r bloomfield_per_cap`, followed by Wellington with `r wellington_per_cap`. In all other wards STRs account for between `r min_per_cap` and `r max_per_cap` of total dwellings.
...
eh_3_bedroom <- 
  property %>% 
  filter(housing, created <= end_2021, scraped >= start_2021, 
...
Table \@ref(tab:tab-1-2) provides the distribution of the listings by listing type in 2021. The wide majority of STRs in Prince Edward County are entire homes, a category which includes single-family homes, townhouses, apartments and condominiums. More than half (`r three_bedrooms`) of these were three-or-more-bedroom housing units, and a quarter (`r two_bedrooms`) were two-bedrooms units. `r one_bedrooms` had one bedroom, and only `r studios` were studios. In general, three-or-more-bedroom units are over-represented on STR platforms in comparison with the County’s overall housing stock.
...
In 2021 entire-home listings accounted for `r eh_listings` of all daily active listings, and `r eh_revenue` of total host revenue. (Private rooms accounted for nearly all of the remainder.) Moreover, the dominance of entire-home listings in Prince Edward County’s STR market is increasing over time.
...
Since 2018, the number of active STRs listings has been steadily decreasing in Prince Edward County. Figure \@ref(fig:fig-1-3) shows the change in active listings and revenue relative to one year earlier, which is a convenient way to remove seasonal variation to identify underlying growth trends. The figure indicates that, since 2018, the number of daily active listings has been declining. Revenues, on the other hand, have experienced growth since the beginning of 2019. With the exception of the winter 2021 months, Prince Edward County's STR revenues have been higher than the year before. The pandemic had a clear negative effect on the number of active daily listings being displayed as reserved or available on STR platforms, but an opposite, slightly positive effect on revenues.
...
listing_growth_2018 <- 
  daily %>% 
  filter(housing, status != "B", date >= start_2020 - years(2),
         date <= end_2020 - years(1)) %>% 
  group_by(year_2018 = date >= start_2020 - years(1)) %>% 
  summarize(n = n() / 365) %>% 
...
listing_growth_2021 <- 
  ...
  filter(housing, status != "B", date >= start_2020,
         date <= end_2021,
         (date <= max(daily$date) - years(1) | date >= start_2021),
         date != "2020-02-29") %>%
  group_by(year_2021 = date >= start_2021) %>% 
  summarize(n = n() / as.numeric((max(daily$date) - as.Date("2021-01-01")))) %>% 
  summarize(change = (n[2] - n[1]) / n[1])  %>% 
  pull(change) %>% 
  scales::percent(0.1)
...
Overall, the year-over-year change in average active listings from 2017-2018 to 2018-2019  was `r listing_growth_2018` and the year-over-year change from 2017-2018 to 2018-2019 was `r listing_growth_2018`. In the 2019-2020 and 2020-2021 periods, because of the Covid-19 pandemic, the year-over-year change in active daily listings were `r listing_growth_2020` and `r listing_growth_2021`, respectively. The 2020-2021 period is marked by a sharper decline in the number of active listings than the year before, which could be due to both the pandemic and the moratorium on STA registrations.

``` {r para_7, cache = TRUE, cache.lazy = FALSE, cache.extra = mtime(here("output", "str_processed.qsm"))}

qload(here("output", "str_processed.qsm"), nthreads = availableCores())

# YOY reservation change, 2020-2021
reservation_change_2021 <- 
  daily %>%
  filter(housing, status == "R", date >= start_2021 - years(1),
         date <= end_2021) %>%
  group_by(year_2021 = date >= start_2021) %>%
  summarize(n = n()) %>%
  summarize(change = (n[2] - n[1]) / n[1]) %>% 
...
# YOY reservation change, 2018-2020
...
# YOY revenue change, 2020-2021
active_yoy_revenue_2021 <- 
  daily %>%
  filter(housing, status == "R", date >= start_2020,
         date <= end_2020 + years(1),
         (date <= "2020-07-31" | date >= start_2020 + years(1)),
         date != "2020-02-29") %>%
  group_by(year_2021 = date >= start_2020 + years(1)) %>%
...
reserved_nights_decrease_2021 <- 
  ...
  filter(housing, status == "R", date >= start_2021 - years(1), 
         date <= end_2021) %>% 
  count(date_2020 = date >= start_2021) %>% 
  summarize(growth = (n[2] - n[1]) / n[1]) %>% 
  pull(growth) %>% 
  scales::percent(0.1)
...
reserved_nights_increase_jan_feb <- 
  ...
  filter(housing, status == "R", date >= start_2020, 
         month(date) %in% c(1, 2)) %>% 
  count(date_2021 = date >= start_2021) %>% 
  summarize(growth = (n[2] - n[1]) / n[1]) %>% 
  pull(growth) %>% 
  scales::percent(0.1)
...
trends <-
  tibble(
    date = as.Date(c("2021-03-16", "2021-04-16", "2021-05-16", "2021-06-16",
                     "2021-07-16", "2021-08-16", "2021-09-16", "2021-10-16",
                     "2021-11-16", "2021-12-16", "2021-12-31")),
...
reservations <- 
  ...
  filter(status == "R") %>% 
  mutate(n = slide_dbl(n, mean, .before = 6)) %>% 
  filter(date >= "2020-01-01", date <= "2021-02-29") %>% 
  select(-status) %>% 
...
``` {r make_fig_1_5}
 ...
 figure_1_5_fun <- function(regular = "", condensed = "") {
   ...
   reservations %>% 
     ...
     geom_text(aes(x = as.Date("2021-12-31"), 
                   y = mean(value[date == as.Date("2021-12-31")]),
                   label = paste(
                     prettyNum(round(abs(diff(
                       value[date == as.Date("2021-12-31")])), -1), ","),
                     "fewer", "reservations", "than", "expected", sep = "\n")), 
               inherit.aes = FALSE, hjust = 1, family = condensed, 
               nudge_x = -4) +
     geom_segment(aes(x = as.Date("2021-12-31"), xend = as.Date("2021-12-31"),
                      y = min(value[date == as.Date("2021-12-31")]),
                      yend = max(value[date == as.Date("2021-12-31")])),
...
Figure \@ref(fig:fig-1-5) provides a closer look at daily reservations since 2020, comparing the actual trajectory of reservations during the pandemic with what the trajectory of reservations would have been expected to be, based on previous growth in Prince Edward County's STR market but in the absence of the pandemic. (To do this, we use seasonal decomposition to identify the regular seasonal fluctuations in STR activity and separate them from the underlying patterns of growth or decline.) In total, from March through December 2021, we estimate that there have been `r res_total_pandemic_dif` fewer STR nights reserved than would normally have been expected to occur. The `r res_total_pandemic` total nights reserved in this time period is only `r res_total_pandemic_pct` of the `r res_total_pandemic_trend` nights total that would represent the previous growth trend.
...
STR host revenue decreased by a staggering `r active_yoy_revenue` in 2021 compared to 2020. To offer some perspective, from 2018 to 2020, STR revenue increased by `r active_yoy_revenue19`. Throughout the May-December period of 2021, moreover, nightly prices have been an average of `r nightly_price_dif` lower than expected. Spread across the `r res_total_pandemic` nights reserved during this period, this means that STR operators collectively earned `r total_price_dif` less than they would have on their bookings in the absence of the pandemic. When the lower prices on reservations which did occur is combined with the reservations which did not occur, our estimate is that Prince Edward County’s STR hosts lost a total of `r total_revenue_lost` million in revenue between March and December 2021 because of the Covid-19 pandemic.
...
diff_host_250 <- 
  n_hosts_over_250_20 - n_hosts_over_250
...
Among all the STR hosts who earned revenue in the City of Prince Edward County in 2020-2021, the median revenue was `r median_host_rev`. Throughout the City of Prince Edward County, there were `r n_hosts_over_250` hosts that earned more than $250,000 in 2021, which is `r diff_host_250` less hosts than in 2020. Figure \@ref(fig:fig-1-6) shows the percentage of the total `r annual_rev` in STR revenue which accrued to each decile of hosts. The figure shows that revenue is disproportionately concentrated among a small number of hosts; the top 10% earned `r host_rev_top_10`: the top 5% earned `r host_rev_top_5` of revenue, while the top 1% of hosts earned `r host_rev_top_1` of all revenue. The pandemic has thus not impacted greatly the sharp difference in revenue distribution among STR hosts. To put this revenue distribution in context, Montreal’s top 10% of STR hosts earned 68.8% of all revenue in 2020. On the other hand, Vancouver's top 10% hosts only earned 43.2% of all revenue in 2020.
...
figure_1_7_fun <- function(regular = "", condensed = "") {
  ...
  ML %>% 
    filter(date >= "2017-01-01", date <= "2021-11-30") %>% 
    ggplot() +
...
In 2021, `r ml_listings` of active listings in Prince Edward County were multilistings, earning `r ml_revenue` of total host revenue. Multilistings had been growing steadily since 2017, both in terms of listings and revenue percentage, until the Covid-19 pandemic, when their proportion dropped somewhat (Figure \@ref(fig:fig-1-7)). Still, through the fall of 2021 multilistings were earning a bit more than 4 out of every 10 dollars on STR platforms in Prince Edward County. 
...
``` {r para_9_1, cache = TRUE, cache.lazy = FALSE, cache.extra = mtime(here("output", "national_comparison.qs"))}
 ...
 to_active_per_1000 <- 
   national_comparison %>% 
   filter(city == "Prince Edward County") %>% 
   pull(listings_per_1000) %>% 
   round(1) %>% 
   prettyNum(",")
...
<!-- In 2021, Prince Edward County had the largest STR market in the country by both active listing numbers (`r avg_listings_active`) and host revenue (`r annual_rev`), followed in both cases by Montreal and Vancouver (Table \@ref(tab:tab-1-3)). However, in relative terms, Vancouver stands considerably ahead of both Prince Edward County and Montreal. Vancouver had the most active listings per 1000 households (`r van_active_per_1000` compared to `r to_active_per_1000` in Prince Edward County) and the most revenue per listing (`r van_rev_per_listing` compared to `r to_rev_per_listing` in Prince Edward County). The pandemic has hit the STR markets of the ten largest Canadian metropolises differently. Small markets such as Brampton and Hamilton have seen their daily active listings and yearly revenue decrease less than bigger markets. On the other hand, both Montreal and Vancouver have experienced drastic drops in revenue, by `r mtl_rev_yoy` and `r van_rev_yoy` respectively. -->
...
national_comparison %>% 
   ...
   kbl(caption = "2021 STR activity in the largest ten Canadian municipalities",
       align = "lrrrr") %>%
   kable_styling(latex_options="scale_down") %>%
